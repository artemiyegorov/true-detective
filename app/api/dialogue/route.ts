...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      // non-JSON. Strip JSON braces / fences. CRUCIALLY, also screen for
      // the model's meta-thinking preambles ("I'm ready to begin the
      // interrogation as Martin Reyes…", "I'll respond in character…",
      // "I have my secrets X, Y, Z…") because the fallback otherwise
      // leaks the entire system prompt to the player. If the raw text
      // looks like LLM meta narration, replace it with a safe placeholder
      // so we never render a leak.
      const cleaned = text
        .replace(/```[\s\S]*?```/g, "")
        .replace(/[{}\[\]]/g, "")
        .replace(/\s+/g, " ")
        .trim();
      const looksLikeMeta =
        /\bI(?:'m| am) ready (?:to begin|for)\b/i.test(cleaned) ||
        /\bI(?:'ll| will) respond in character\b/i.test(cleaned) ||
        /\bdetective's first message\b/i.test(cleaned) ||
        /\bin character\b.*\b(?:nervous|polite|cautious)\b/i.test(cleaned) ||
        /\bI have my secrets\b/i.test(cleaned) ||
        /\bplease (?:give|provide) me the detective\b/i.test(cleaned) ||
        /\bawaiting (?:the )?detective/i.test(cleaned) ||
        /\bsystem prompt\b/i.test(cleaned) ||
        /\bcharacter prompt\b/i.test(cleaned);
      const fallbackVoice = looksLikeMeta
        ? "…"
        : cleaned.slice(0, 500);
      json = {
