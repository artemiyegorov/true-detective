...
**Pipeline:** `tom-warm.jpg` is the canonical hero portrait, hand-curated (saved from ChatGPT or upstream art tool — not regenerated by the script). Identity, wardrobe, setting, lighting, color grading all locked there. The 3 mood variants are produced by **OpenAI gpt-image-2 edit** (`openai/gpt-image-2/edit` on fal.ai), passing tom-warm.jpg as the reference + a per-variant prompt structured as the two-column pattern recommended by OpenAI: `Change only:` + `Preserve:`. The detailed Preserve list (repeated every iteration) is what prevents identity drift. The script `scripts/generate-tom-portraits.mjs` reads the prompts directly from this file.

#### Tom — preserve list
*Repeated verbatim in every variant. Keep this list comprehensive and unchanged between variants — the gpt-image-2 prompting guide explicitly says: "repeat the preserve list each iteration to reduce drift". Note: hand position is intentionally NOT in this list — each variant repositions the hands to match the emotion.*
```
Face geometry, all facial features (eyes, nose, mouth, jawline), identity, head shape, hair color and hairstyle, skin tone and texture, eye color, every wrinkle, the exact wardrobe (navy blazer over light-blue button-down shirt, top button undone, no tie), the wedding ring on the left ring finger, the wristwatch on the left wrist, the seated upright body position behind the dark wooden table, the concrete textured wall background, the dramatic warm tungsten key light from upper-left, the cinematic color grading, film grain, depth of field, framing and crop, camera angle.

No redesign of facial features. No wardrobe changes. No background changes. No lighting shift. No camera angle shift. No new objects added. Match the photographic style of the reference exactly.
```

#### Tom — variant: guarded (under questioning, mask thinning)
*Change-only block. The script prepends "Change only:" and appends the preserve list as "Preserve:".*
```
The facial expression and the position of the hands. Remove the smile entirely. The mouth is now a flat neutral line with lips firmly pressed together. The jaw is clearly tightened with the masseter muscle visible. Brows are slightly lowered. The gaze is hard and direct into the camera. There is a faint sheen of sweat at the temple. The posture is rigid with shoulders squared. The face of a man under interrogation pressure but still composed.

Hands: both hands are now clenched into tight fists on the table surface, knuckles slightly white from the pressure, the wedding ring still visible on the left ring finger. Forearms tense.
```

#### Tom — variant: cracking (vulnerability shows through)
*Change-only block.*
```
The facial expression and the position of the hands. Remove the smile entirely. The mouth is slightly parted with the lower lip just barely trembling. The brows are pulled together and up in distress with a worried furrow between them. The eyes are glassy and unfocused, looking down and to the lower-left, NOT at the camera. The whites of the eyes are slightly red and shining with held-back emotion. Visible exhaustion under the eyes.

Hands: the right hand has come up off the table to his temple — the fingers spread into the hair on the side of his head, as if holding his head together. The left hand still rests on the table surface, palm down, fingers limp. The wedding ring is visible on the left ring finger.
```

#### Tom — variant: broken (final breakdown)
*Change-only block.*
```
The facial expression, the posture, and the position of the hands. Remove the smile entirely. The mouth is slack and slightly open, the jaw loose.

The gaze: eyes wide open (NOT closed, NOT downcast at the table). The eyes look through and past the camera at empty space — a thousand-yard stare into nothing. The pupils are focused on the middle distance, lost in thought. He is NOT making eye contact with the viewer and NOT looking down at the table.

The eyes are visibly wet: a reflective film of tears coats the surface of each eye, catching the warm tungsten key light as bright highlights on the lower eyelids. A single tear has escaped the lower eyelash of one eye and is suspended mid-cheek as a small glistening droplet just below the lash line. Whites of the eyes are bloodshot with fine red veins visible. The skin under both eyes is darker and slightly puffy.

Hands: both hands have fallen limp on the table, palms turned slightly upward, fingers loose and softly curled — defeated, no tension left in them. The wedding ring is visible on the left ring finger.

The brows are neutral, slightly raised at the inner corners in a defeated furrow. The skin tone is paler and more drained than in the reference, with a faint grayness around the mouth. The chin is dropped a centimeter or two. The shoulders are collapsed forward, the navy blazer slightly bunched at the shoulders. The complete absence of energy. A man whose life is over.
```
